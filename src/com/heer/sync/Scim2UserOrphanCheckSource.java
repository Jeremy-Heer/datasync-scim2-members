...
/**
 * Custom SCIM2 source for detecting orphaned users.
 * <p>
 * This source plugin queries the SCIM2 directory during resync operations
 * and streams all users through the sync pipe. It's designed to work with
 * an LDAP destination plugin that checks for user existence in LDAP and
 * deletes orphaned users from SCIM2 if they're not found.
 * </p>
 * 
 * <h2>Architecture:</h2>
 * <pre>
 * SCIM2 Source → Sync Pipe → LDAP Destination → Orphan Cleanup Plugin
 * (list all users)          (lookup in LDAP)    (delete if not found)
 * </pre>
 * 
 * <h2>Usage:</h2>
 ...
 * @author Jeremy Heer
 */
public class Scim2UserOrphanCheckSource extends SyncSource
{
  ...
  {
    return "SCIM2 User Orphan Check Source";
  }
  ...
  public String[] getExtensionDescription()
  {
    return new String[] {
      ...
      "",
      "This source queries SCIM2 and streams all users through the sync pipe.",
      "Used with LDAP destination to detect and delete users not found in LDAP.",
      "",
      "Features:",
      "- Paginated SCIM2 queries for scalability",
      "- Preserves scim2UserId attribute for cleanup",
      "- Converts SCIM2 UserResource to LDAP Entry format",
  ...
  public void defineConfigArguments(final ArgumentParser parser) throws ArgumentException
  {
    ...
    parser.addArgument(new StringArgument(
        ...
        "{dn}",
        "Base DN for synthetic LDAP entries (e.g., ou=Users,dc=example,dc=com). " +
        "Can be provided via config file as ldap.base.dn."));
    
    // User ID attribute
    parser.addArgument(new StringArgument(
        null, "user-id-attribute", false, 1,
        "{attr}",
        "LDAP attribute for user ID (e.g., uid). Can be provided via config file as user.id.attribute."));
    
    // Page size for SCIM2 queries
    parser.addArgument(new StringArgument(
        null, "page-size", false, 1,
        "{size}",
        "Number of users to fetch per page from SCIM2 (default: 100). " +
      ...
      final ArgumentParser parser)
  {
    ...
    // Get base DN
    baseDN = getConfigValue(parser, "base-dn", "ldap.base.dn", "ou=Users,dc=example,dc=com");
    
    // Get user ID attribute
    userIdAttribute = getConfigValue(parser, "user-id-attribute", "user.id.attribute", "uid");
    ...
    serverContext.debugInfo("SCIM2 User Orphan Check Source initialized successfully");
    ...
    serverContext.debugInfo("  User ID attr: " + userIdAttribute);
  ...
  {
    serverContext.debugInfo("Finalizing SCIM2 User Orphan Check Source");
  ...
  /**
   * Lists all users from SCIM2 during resync operations.
   * Users are fetched in pages and converted to LDAP Entry format,
   * preserving the scim2UserId attribute for later cleanup.
   */
  @Override
  public void listAllEntries(final BlockingQueue<ChangeRecord> outputQueue) throws EndpointException
  {
    serverContext.debugInfo("Starting SCIM2 user enumeration for orphan detection");
    ...
    try
    {
      while (hasMore)
      {
        serverContext.debugInfo("Fetching SCIM2 users page (startIndex=" + startIndex + 
                               ", pageSize=" + pageSize + ")");
        
        // Query SCIM2 for users
        ListResponse<UserResource> response = scimService.searchRequest(userBasePath)
            .page(startIndex, pageSize)
            .invoke(UserResource.class);
        
        List<UserResource> users = response.getResources();
        ...
        // Convert each SCIM2 user to LDAP entry and queue for processing
        for (UserResource user : users)
        {
          Entry entry = convertUserToLdapEntry(user);
        ...
        serverContext.debugInfo("Queued " + users.size() + " users (total so far: " + totalUsers + ")");
      ...
      serverContext.debugInfo("Completed SCIM2 user enumeration: " + totalUsers + " users queued");
    }
    catch (InterruptedException e)
    {
      Thread.currentThread().interrupt();
      throw new EndpointException(PostStepResult.ABORT_OPERATION,
                                  "User enumeration interrupted", e);
    }
    catch (Exception e)
    {
      throw new EndpointException(PostStepResult.ABORT_OPERATION,
                                  "Failed to list SCIM2 users: " + e.getMessage(), e);
    }
  }
  
  /**
   * Converts a SCIM2 UserResource to an LDAP Entry format.
   * Preserves the SCIM2 user ID in the scim2UserId attribute for later cleanup.
   */
  private Entry convertUserToLdapEntry(final UserResource user) throws Exception
  {
    String userName = user.getUserName();
    String scim2UserId = user.getId();
    
    // Build DN (simple string concatenation)
    String dn = userIdAttribute + "=" + userName + "," + baseDN;
    
    // Create entry
    Entry entry = new Entry(dn);
    entry.addAttribute("objectClass", "inetOrgPerson");
    entry.addAttribute(userIdAttribute, userName);
    entry.addAttribute("scim2UserId", scim2UserId); // CRITICAL: Preserve for cleanup
    
    // Add common attributes if available
    if (user.getName() != null)
    {
      if (user.getName().getFormatted() != null) {
        entry.addAttribute("cn", user.getName().getFormatted());
      }
      if (user.getName().getFamilyName() != null) {
        entry.addAttribute("sn", user.getName().getFamilyName());
      }
      if (user.getName().getGivenName() != null) {
        entry.addAttribute("givenName", user.getName().getGivenName());
      }
    }
    
    if (user.getDisplayName() != null) {
      entry.addAttribute("displayName", user.getDisplayName());
    }
    
    if (user.getEmails() != null && !user.getEmails().isEmpty()) {
      entry.addAttribute("mail", user.getEmails().get(0).getValue());
    }
